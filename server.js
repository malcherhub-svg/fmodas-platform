...
app.post('/api/products', requireAuth, requireAdmin, upload.array('photos', 8), (req, res) => { const { name, description = '', category, gender = 'Unissex', costPrice, salePrice, stock = 0 } = req.body; if (!name || !category || !salePrice) return res.status(400).json({ error: 'Nome, categoria e preço de venda são obrigatórios.' }); const photos = (req.files || []).map(file => `/uploads/${file.filename}`); const result = db.prepare('INSERT INTO products (name,description,category,gender,cost_price,sale_price,stock,photos_json) VALUES (?,?,?,?,?,?,?,?)').run(name, description, category, gender, cents(costPrice || 0), cents(salePrice), Number(stock), JSON.stringify(photos)); res.status(201).json({ product: publicProduct(db.prepare('SELECT * FROM products WHERE id=?').get(result.lastInsertRowid)) }); });
...
app.get('*', (_req, res) => res.sendFile(path.join(__dirname, 'cliente.html')));
